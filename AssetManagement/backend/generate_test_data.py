...
CATEGORIES = [
    ...
    ("網路設備", ["Cisco", "MikroTik", "Ubiquiti"],
     ["Catalyst 9200", "CRS326", "UniFi USW-Pro-24"], "NETW"),
    ("印表機", ["HP", "Brother", "Epson"],
     ["LaserJet Pro M404", "HL-L2370DW", "WorkForce Pro WF-7920"], "PRNT"),
    ("辦公傢俱", ["IKEA", "Herman Miller", "Steelcase"],
     ["Bekant 升降桌", "Aeron 人體工學椅", "Gesture 辦公椅"], "FURN"),
    ("空調設備", ["日立", "大金", "三菱電機"],
...
def generate_assets(count=100):
    ...
    for i in range(count):
        # 選擇類別
        cat, brands, models, prefix = random.choice(CATEGORIES)
        ...
        # 資產編號（4碼前綴 + 2碼年份 + 6碼流水號 = 12碼）
        year_suffix = str(purchase_date.year % 100).zfill(2)
        code_num = i + 1
        serial = str(code_num).zfill(6)
        asset_code = f"{prefix}{year_suffix}{serial}"
